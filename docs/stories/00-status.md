...
| 8 | [symmetric cipher (AEAD)](runtime-v2/08-symmetric-cipher.md) | 🔄 **in-progress** — the **first rung of the TLS ladder** (gates rv2 9). **Phases A + B LANDED 2026-09-08**: A ChaCha20-Poly1305 (ids 111/112, RFC 8439 §2.8.2 byte-exact); B AES-128/256-GCM (ids 113/114) via **AES-NI + PCLMULQDQ**, CPUID-gated + target-attributed (portable binary), NIST SP 800-38D cases 4 & 16 byte-exact. All hand-rolled, constant-time, KAT-gated in test_crypto (**36/0**), ASan/UBSan clean. Remaining: C software AES + ARMv8 path → D cookie wrapper → E gate. Consumers: rv2 9 TLS + porch encrypted cookies |
